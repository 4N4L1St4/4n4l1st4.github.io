...
Opa\! Já te enviei a tabela em Markdown na resposta anterior. É só copiar e colar o bloco de código:

```markdown
| Nome da regra                                   | Categoria base Mitre                               | Tecnologia aplicada                                   |
| :---------------------------------------------- | :------------------------------------------------- | :------------------------------------------------------ |
| Tentativas Múltiplas de Login Falhas (Brute Force) | Initial Access (T1078) / Credential Access (T1110) | Logs de Autenticação (Active Directory, Linux Auth Logs, Aplicações Web, VPN) |
| Login de Localização Geográfica Incomum         | Initial Access (T1078) / Defense Evasion (T1564)   | Logs de Login (VPN, Cloud Services - O365, Azure AD, GSuite) |
| Grande Volume de Dados Transferidos (Exfiltração) | Exfiltration (T1041, T1048, T1537)                 | Logs de Proxy/Firewall (Web Proxy, Firewall, DLP), Logs de Cloud Storage (S3, Azure Blob) |
| Criação/Modificação de Conta Privilegiada Suspeita | Privilege Escalation (T1078, T1098, T1136)          | Logs de Auditoria de SO (Windows Event Logs - Security, Linux Auditd), Logs de Active Directory/LDAP |
| Conexão Suspeita para IP Malicioso Conhecido    | Command and Control (T1071, T1573) / Execution (T1204) | Logs de DNS, Logs de Firewall/Proxy, Threat Intelligence Feeds |
```